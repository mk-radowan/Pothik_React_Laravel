...
var v = class {
  ...
  static reset(e) {
    ...
    }), this.save(e), window.location.hash && setTimeout(() => document.getElementById(window.location.hash.slice(1))?.scrollIntoView());
  }
  static restore(e) {
...
var ee = class {
  ...
  restore(e) {
    if (!M) return this.initialState?.[this.rememberedState]?.[e];
  }
  pushState(e, t = null) {
...
var re = class {
  constructor() {
    typeof window < "u" && window?.performance.getEntriesByType("navigation").length > 0 ? this.type = window.performance.getEntriesByType("navigation")[0].type : this.type = "navigate";
  }
  get() {
    return this.type;
  }
...
var Je = { buildDOMElement(r) {
  ...
  Array.from(document.head.childNodes).filter((i) => this.isInertiaManagedElement(i)).forEach((i) => {
    ...
    a && !i.isEqualNode(a) && i?.parentNode?.replaceChild(a, i);
...
var _e = (r) => {
  ...
  return t.style.transition = "all 0 linear", t.style.transform = `translate3d(${i}%,0,0)`, f.showSpinner || e.querySelector(f.spinnerSelector)?.remove(), s !== document.body && s.classList.add(`${p}-custom-parent`), s.appendChild(e), e;
};
var Oe = () => Ze(f.parent) ? f.parent : document.querySelector(f.parent);
var Ue = () => {
  document.documentElement.classList.remove(`${p}-busy`), Oe().classList.remove(`${p}-custom-parent`), document.getElementById(p)?.remove();
};
...
function ot(r) {
  b.isStarted() && r.detail.progress?.percentage && b.set(Math.max(b.status, r.detail.progress.percentage / 100 * 0.9));
}
function at(r, e) {
...
function Me(r) {
  ...
  return !(r.target && (r?.target).isContentEditable || r.defaultPrevented || e && r.which > 1 || e && r.altKey || e && r.ctrlKey || e && r.metaKey || e && r.shiftKey || e && "button" in r && r.button !== 0);
}
var Gr = new G();
...
function pe2(p2 = {}) {
  let r = typeof window > "u" ? null : Gr.getCached(window.location.pathname, p2), s = typeof window > "u" ? null : Gr.getPrefetching(window.location.pathname, p2), [a, i] = (0, import_react12.useState)(r?.staleTimestamp || null), [P2, l] = (0, import_react12.useState)(s !== null), [m, g] = (0, import_react12.useState)(r !== null);
...
var ce = ({ children: p2, data: r, params: s, buffer: a, as: i, always: P2, fallback: l }) => {
  ...
  return (0, import_react13.useEffect)(() => {
    if (c.current) return e.current = new IntersectionObserver((h) => {
      if (!h[0].isIntersecting || (P2 || e.current?.disconnect(), y2)) return;
      f2(true);
      let S = n2();
      Gr.reload({ ...S, onStart: (R2) => {
        f2(true), S.onStart?.(R2);
      }, onFinish: (R2) => {
        g(true), f2(false), S.onFinish?.(R2);
      } });
